...
var dayVideo = require('../img/OUNM9270.m4v');

var MasterBedroom = React.createClass({
  propTypes: {
      ...
      <ViroNode >
          <ViroAnimatedComponent animation="fadeIn" run={true} loop={false}>
              <ViroNode opacity={0.0}>
                  <Viro360Video source={dayVideo} paused={this.state.videoPaused} loop={false} volume={0.0} rotation={[0,-180,0]}/>
                  ...
                  <PortalElement  backPortal={true}  iconOffset={1.75} roomTitleLength={1} scale={[.8,.8,1]} roomTitleText="Master Bathroom"
                                  position={polarToCartesian([-10, -120, 0])} jumpToRoom="master_bath_room" showDayImage={this.props.showDayImage} onClick={this.props.showNextRoom}/>
                  ...
                  <ActionElement  backPortal={true}  iconOffset={2.25} titleLength={1.25} scale={[.8,.8,1]} titleText="Close Window Shades"
                                  position={polarToCartesian([-10, -150, 0])} onClick={this._closeWindows}/>)}
              </ViroNode>
          </ViroAnimatedComponent>
      </ViroNode>
  );
